...
const login = async function (ctx, next) {
  ...
  if (user != null && user.password === post.password) {
    ...
    ctx.status = 200
    ctx.body = V.loginPage('success', post.user)
    // console.log(post, ' login success!')
    // ctx.redirect('/login?op=success')
  } else {
    ctx.status = 401
    ctx.body = V.loginPage('fail', null)
    // ctx.redirect('/login?op=fail')
  }
}

var logout = async function (ctx, next) {
  if (ctx.session.user) delete ctx.session.user
  ctx.redirect('/login?op=logout')
}
